...
export default function NextBreadCrumb({
  omitRoot = false,
  homeIcon = <AiFillHome className="text-warning" />,
  isHomeIcon = true,
  isChevron = true,
  ...
  const getPathFormatLocale = (isReady, pathname) => {
    ...
    const pathsDisplay = pathsLocale.map((v, i, array) => {
      ...
      // 其它中間樣式
      // const clickTarget = paths.slice(0, i + 1)[
      //   paths.slice(0, i + 1).length - 1
      // ]
      return (
        <li key={i} className="breadcrumb-item ">
          <Link
            href={
              // clickTarget == 'member' ||
              // (clickTarget == 'product' && clickTarget == 'figure') ||
              // clickTarget == 'dashboard'
              //   ? paths.slice(0, i + 1).join('/')
              //   : paths.slice(0, i + 2).join('/')
              paths.slice(0, i + 1).join('/')
            }
            className="link-warning fw-semibold text-warning"
          >
            {v}
          </Link>
        </li>
  ...
  return (
    <nav aria-label="breadcrumb">
      <ol
        className={`m-0 breadcrumb px-3 py-3 ${bgClass} rounded-3 ${
